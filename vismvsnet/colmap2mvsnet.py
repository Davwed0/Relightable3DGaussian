...
'''
python colmap2mvsnet.py --dense_folder /home/gj/Project/3DG_material/datasets/nerf/lego/colmap --max_d 256
'''
...
#
    parser.add_argument('--dense_folder', type=str, help='Project dir.' ,
                        default='/home/gj/Project/3DG_material/datasets/neilfpp/data_dtu/DTU_scan1/colmap')

    parser.add_argument('--max_d', type=int, default=None)
    parser.add_argument("--depth_min", type=float, default=None)
    parser.add_argument("--depth_interval", type=float, default=None)
    ...
    for i in range(num_images):
        ...
        if args.depth_min is None:
            ...
            for p3d_id in images[i+1].point3D_ids:
                ...
                # For HDR dataset (Bound box)
                # print("Using bbx for HDR dataset!!!")
                # pt_x = points3d[p3d_id].xyz[0]
                # pt_y = points3d[p3d_id].xyz[1]
                # pt_z = points3d[p3d_id].xyz[2]
                # skip = [pt_x > 0.164964 + 2.1281 / 2, pt_x < 0.164964 - 2.1281 / 2,
                #         pt_y > -0.131760 + 1.62515 / 2, pt_y < -0.131760 - 1.62515 / 2,
                #         pt_z > 0.227717 + 2.05441 / 2, pt_z < 0.227717 - 2.05441 / 2]
            ...
            depth_min = zs_sorted[int(len(zs) * 0.01)] # 0.01
            depth_max = zs_sorted[int(len(zs) * 0.99)] # 0.99
        else:
        ...
        if args.depth_interval is None:
    ...
    depth_range_max = max(d[3] - d[0] for d in depth_ranges.values())
    print("max_depth_range", depth_range_max)
    ...
    for i,x in enumerate(queue):
        print ("Running "+str(i)+" from "+str(len(queue)))
        result.append(calc_score(x))
    for i, j, s in result:
        score[i, j] = s
        score[j, i] = s
    view_sel = []
    ...
    #     else:
    #         if os.path.join(image_dir, images[i+1].name) != os.path.join(renamed_dir, '%08d.jpg' % i):
    #             shutil.copyfile(os.path.join(image_dir, images[i+1].name), os.path.join(renamed_dir, '%08d.jpg' % i))
    #         else:
    #             print("the filename: "+images[i+1].name+" was not recreated")
