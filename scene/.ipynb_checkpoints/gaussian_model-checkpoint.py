...
from utils.sh_utils import RGB2SH, eval_sh, SH2RGB
...
class GaussianModel:
    ...
    def prune_points(self, mask):
        black_mask = self.mask_black_gaussians()
        mask = np.logical_or(black_mask, mask)
    ...
    def mask_black_gaussians(self):
        shs_dc = self._shs_dc.detach().cpu().numpy()
        colors = shs_dc.squeeze(1)
        colors = SH2RGB(colors)
        original_indices = len(colors)

        black_indices = np.where(np.mean(colors, axis=1) == 0)[0]
        mask = np.setdiff1d(np.arange(self._xyz.shape[0]), black_indices)

        print(f"Removed {len(black_indices)} black gaussians out of {original_indices}")
