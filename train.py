...
def decimate_near_black_pointcloud(pcd):
    color_magnitude = np.linalg.norm(np.asarray(pcd.colors), axis=1)
    valid_color_mask = color_magnitude > 0.02  # Threshold for "almost black"
    
    # Filter out black points
    fused_point_cloud = np.asarray(pcd.points)[valid_color_mask]
    fused_normal = np.asarray(pcd.normals)[valid_color_mask]
    fused_color = np.asarray(pcd.colors)[valid_color_mask]
    
    print(f"Removed {np.sum(~valid_color_mask)} black/almost black points")
    print("Number of points after black point removal: ", fused_point_cloud.shape[0])

    return BasicPointCloud(
        points=fused_point_cloud,
        normals=fused_normal, 
        colors=fused_color
    )


def training(dataset: ModelParams, opt: OptimizationParams, pipe: PipelineParams, is_pbr=False, start_iter=None):
    first_iter = 0
...
if __name__ == "__main__":
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), is_pbr=is_pbr, start_iter=args.start_iter)
